...
function cleanJson(text: string): TutorReply {
  ...
  return {
    ...
    new_words: Array.isArray(parsed.new_words)
      ? parsed.new_words.slice(0, 3).map((item: any) => ({
...
async function openAiJson(apiKey: string, path: string, body: any) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`OpenAI ${path}: ${response.status} ${message.slice(0, 500)}`);
  }
...
async function transcribe(apiKey: string, audioBase64: string, mimeType: string) {
  ...
  form.append('file', new Blob([new Uint8Array(buffer)], { type: mimeType }), `learner.${extension}`);
  ...
  if (!response.ok) {
    const message = await response.text();
    throw new Error(`OpenAI transcription: ${response.status} ${message.slice(0, 500)}`);
  }
...
async function synthesize(apiKey: string, text: string, level: string) {
  const response = await fetch('https://api.openai.com/v1/audio/speech', {
    ...
    body: JSON.stringify({
      ...
      instructions: 'Warm, patient English conversation coach. Clear pronunciation, friendly and natural, never robotic.',
    }),
  });
  if (!response.ok) {
    const message = await response.text();
    throw new Error(`OpenAI speech: ${response.status} ${message.slice(0, 500)}`);
  }
...
export const conversationTurn = onCall(
  ...
  async (request) => {
    ...
    const typedText = typeof data.text === 'string' ? data.text.trim().slice(0, 1500) : '';
    const mimeType = typeof data.mimeType === 'string' ? data.mimeType : 'audio/m4a';
    const requestedLevel = ['A1', 'A2', 'B1'].includes(data.level) ? data.level : 'A1';
    const requestedTopic = typeof data.topic === 'string' ? data.topic.slice(0, 80) : 'Anything';
    const seconds = Math.max(0, Math.min(60, Number(data.seconds || 0)));

    if (!typedText && !audioBase64) {
      throw new HttpsError('invalid-argument', 'Audio or text is required.');
    }
    if (audioBase64.length > 6_000_000) {
      throw new HttpsError('invalid-argument', 'Audio turn is too long. Keep turns under about 45 seconds.');
    }

    const apiKey = OPENAI_API_KEY.value();
    const transcript = typedText || (await transcribe(apiKey, audioBase64, mimeType));
    if (!transcript) throw new HttpsError('invalid-argument', 'I could not hear any words. Please try again.');

    const turnsRef = db.collection('users').doc(uid).collection('turns');
    const recentSnap = await turnsRef.orderBy('createdAt', 'desc').limit(6).get();
    const recent = recentSnap.docs.reverse().map((doc) => {
      const t = doc.data();
    ...
    try {
      ...
      tutor = {
        reply: extractResponseText(response) || 'Great. Tell me a little more about that.',
        correction: null,
        explanation_es: null,
    ...
    return {
      ...
      tipEs: tutor.tip_es,
      level: tutor.level,
      newWords: tutor.new_words.map((item) => ({ word: item.word, meaningEs: item.meaning_es })),
      audioBase64: audio,
    };
  },
);
